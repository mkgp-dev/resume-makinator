...
import type { AchievementItem, SharedTemplateConfig, TemplateId } from "@/entities/resume/types"
...
export default function Achievement({ items, config, variant, baseFontSize }: AchievementProps) {
    const isClassic = variant === "classic"
    const bulletIndent = Math.round(baseFontSize * 0.6) + 3
    const itemSpacing = isClassic ? 6 : (config.bulletText ? 0 : 8)
    ...
    return items.map((item, index) => (
        <View key={item.id} style={index === items.length - 1 ? [styles.container, styles.lastChild] : styles.container}>
            ...
            <View style={styles.body}>
                {item.achievementIssuer ? <Text style={styles.subheader}>{item.achievementIssuer}</Text> : null}
